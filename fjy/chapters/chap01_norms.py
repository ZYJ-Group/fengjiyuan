    ...
    with tab2:
        ...
        with col2:
            def draw_norm_frame(p_val):
                ...
                # 3. 【核心修复】：手动锁死边距，禁止自动调整
                # left/right/bottom/top 是相对于画布宽高的比例 (0~1)
                # 这样给标题和坐标轴留出了固定的绝对空间
                ...
                # 6. 标注文本
                # 将文字位置稍微往下调一点，确保它在 top=0.85 的范围内，不要溢出
                ax.text(0, 1.6, status_text, ha='center', fontsize=12, color=color, fontweight='bold')
                ...
                # ⚠️ 绝对不要在循环中调用 fig.tight_layout()，删掉它！
